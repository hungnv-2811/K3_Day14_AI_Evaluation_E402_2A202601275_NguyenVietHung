...
def _coverage(tokens: set[str], reference: set[str]) -> float:
    """Fraction of ``reference`` tokens that also appear in ``tokens``."""
    if not reference:
        return 1.0
...
class RAGASEvaluator:
    ...
    def evaluate_faithfulness(self, answer: str, context: str) -> float:
        ...
        """
        answer_tokens = _tokenize(answer)
        # An empty answer makes no unsupported claim, so nothing can be ungrounded.
    ...
    def evaluate_context_recall(self, contexts: list[str], expected: str) -> float:
        ...
        # Union ignores ordering on purpose: recall asks "was the evidence
        # retrieved at all?", never "was it ranked well?".
    ...
    def evaluate_context_precision(
        ...
        for rank, is_relevant in enumerate(relevant_flags, start=1):
            if is_relevant:
                hits += 1
                precision_sum += hits / rank  # Precision@k at every hit
...
def rerank_by_overlap(contexts: list[str], query: str) -> list[str]:
    ...
    # ``sorted`` is stable, so chunks with equal overlap keep the retriever's
    # original ranking — the reranker only moves better evidence upward.
    return sorted(
        contexts,
...
class LLMJudge:
    ...
    def _build_prompt(
        ...
        """Build a rubric-anchored judge prompt that asks for JSON scores."""
        criteria = "\n".join(
            f"- {name}: {description}" for name, description in rubric.items()
        )
        criteria_keys = ", ".join(f'"{name}": <score>' for name in rubric)
        return f"""You are a strict evaluation judge for a student-services assistant.
Score the answer on each criterion using a 1-5 rubric:
    5 = Correct, complete, grounded in the source policy
    4 = Mostly correct, minor gaps
    3 = Partially correct, some errors
    2 = Significant errors or missing conditions
    1 = Wrong, unsupported, or irrelevant

Judge the substance only. Do NOT reward length, formatting, or confident tone.
...
{{{criteria_keys}}}"""
    ...
    def _parse_scores(self, response: str, rubric: dict[str, Any]) -> dict[str, float]:
        """Parse judge output into criterion → score in [0, 1].

        Accepts either a 0-1 scale or the 1-5 rubric scale (rescaled here).
        Any unparseable criterion falls back to ``DEFAULT_SCORE``.
        """
        ...
        for name in rubric:
            ...
            if isinstance(raw_value, dict):  # e.g. {"score": 4, "reason": "..."}
                raw_value = raw_value.get("score")
            try:
                value = float(raw_value)  # type: ignore[arg-type]
            except (TypeError, ValueError):
                scores[name] = self.DEFAULT_SCORE
                continue
            if value > 1.0:  # 1-5 rubric scale → normalise to 0-1
                value = (value - 1.0) / 4.0
            scores[name] = _clamp(value)
        return scores
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        overall_average = sum(per_response) / len(per_response)
        later = per_response[1:]
        positional_bias = bool(later) and per_response[0] > (
            sum(later) / len(later)
        ) + self.POSITIONAL_BIAS_MARGIN
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            result = evaluator.run_full_eval(
                ...
                # An empty trace means "no retrieval recorded", which must stay
                # None rather than score the retriever 0.0.
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_relevance": _average([r.relevance for r in results]),
            "avg_completeness": _average([r.completeness for r in results]),
            "avg_context_recall": _average(recalls) if recalls else None,
            "avg_context_precision": _average(precisions) if precisions else None,
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        for metric in ANSWER_METRICS:
            new_average = _average([getattr(r, metric) for r in new_results])
            baseline_average = _average([getattr(r, metric) for r in baseline_results])
            report[f"new_avg_{metric}"] = new_average
            report[f"baseline_avg_{metric}"] = baseline_average
            if baseline_average - new_average > REGRESSION_TOLERANCE:
...
ROOT_CAUSE_FAITHFULNESS = "Context is missing or irrelevant — improve retrieval"
ROOT_CAUSE_RELEVANCE = "Answer does not address the question — improve prompt clarity"
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        """
        scores = {
            ROOT_CAUSE_FAITHFULNESS: failure.faithfulness,
            ROOT_CAUSE_RELEVANCE: failure.relevance,
            ROOT_CAUSE_COMPLETENESS: failure.completeness,
        }
        ...
        return min(scores, key=lambda cause: scores[cause])
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures or []):
            fix = (
                ...
                else "Review pipeline stage identified by the root cause"
            )
            lines.append(
                f"| F{index + 1:03d} "
    ...
    def generate_improvement_suggestions(
        ...
        playbook = {
            "hallucination": (
                "Implement a grounding checker that rejects claims whose tokens "
                "are absent from the retrieved contexts"
            ),
            "irrelevant": (
                "Tighten the answer prompt to restate the question and answer "
                "every sub-question explicitly"
            ),
            "incomplete": (
                "Raise top_k and add few-shot examples that preserve dates, "
                "amounts, conditions, and exceptions"
            ),
            "off_topic": (
                "Add intent routing so scope, calendar, and finance questions "
                "hit the right document set"
            ),
            "refusal": (
                "Relax the refusal guardrail so it triggers only when retrieval "
                "returns no in-scope evidence"
            ),
            "unclassified": (
                "Add richer failure labels to the evaluator so unclassified "
                "cases can be clustered"
            ),
        }

        # Highest-count failure types first: fixing one root cause clears a cluster.
        suggestions = [
        ...
        baseline = [
            "Rerank retrieved chunks by query overlap to lift Context Precision "
            "without changing the retrieved set",
            "Add the failing questions to the golden dataset as regression cases",
            "Run the benchmark as a CI quality gate that blocks deploys on a "
            "metric drop larger than 0.05",
        ]
        # Top up with cross-cutting actions so the caller always gets >= 3.
